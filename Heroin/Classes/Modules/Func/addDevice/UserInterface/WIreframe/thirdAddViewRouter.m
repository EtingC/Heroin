...
#import "thirdAddViewRouter.h"
#import "ThirdAddPresenter.h"
#import "ThirdAddViewController.h"

DeclareRoutableView(ThirdAddViewController, thirdAddViewRouter)

@implementation thirdAddViewRouter

//app启动时，注册对应的模块和Router
//不使用+load和+initialize方法，因为在Swift中已经不适用
+ (void)registerRoutableDestination {
    
    [self registerView:[ThirdAddViewController class]];
    [self registerViewProtocol: ZIKRoutable(ThirdAddPresenter)];
    [self registerIdentifier:@"third addView"];
    
}

- (id<ZIKRoutableView>)destinationWithConfiguration:(ZIKViewRouteConfiguration *)configuration {
    
    ThirdAddViewController *destination = [[ThirdAddViewController alloc] init];
